create or replace package body AlimCB as
	...
	begin
		...
		FOR i IN List.FIRST..List.LAST LOOP 
			exist := VerificationValidite(List(i));
			IF(exist != 0) THEN
                		log_pkg.LogInfo('L''id ' || List(i) || ' est valide', 'Ajouter');
				...
				FOR i IN 1 .. ListeActeurs.COUNT LOOP
					...
					EXCEPTION
						WHEN OTHERS THEN log_pkg.LogErreur('Acteur', true, 'Ajouter');
					END;
				END LOOP;
				...
				FOR i IN 1 .. ListeDirectors.COUNT LOOP
					...
					EXCEPTION
						WHEN OTHERS THEN log_pkg.LogErreur('Director', true, 'Ajouter');
					END;
					END LOOP;
				...
				BEGIN
					...
					FOR i IN 1 .. ListeDirectors.COUNT LOOP
						...
						EXCEPTION
							...
							WHEN OTHERS THEN  rollback to SaveDirectors; log_pkg.LogErreur('Movie_Director: Le film ne sera pas ajouté', true, 'Ajouter'); commit; raise;
						END;
					END LOOP;
					...
					FOR i IN 1 .. ListeActeurs.COUNT LOOP
						...
						EXCEPTION
							WHEN OTHERS THEN  rollback to SaveDirectors; log_pkg.LogErreur('Movie_Actor: Le film ne sera pas ajouté', true, 'Ajouter'); commit; raise;
						END;
					END LOOP;
					...
					FOR i IN 1 .. ListeGenres.COUNT LOOP
						...
						EXCEPTION
							WHEN OTHERS THEN rollback to SaveDirectors; log_pkg.LogErreur('Movie_Genre: Le film ne sera pas ajouté', true, 'Ajouter'); commit; raise;
						END;
					END LOOP;
				EXCEPTION
			
					WHEN OTHERS THEN rollback to SaveDirectors; log_pkg.LogErreur('Film', true, 'Ajouter'); commit; raise;
				END;
				...
				log_pkg.LogInfo('Insert Movie_genre: OK', 'Ajouter');
                		...
                		log_pkg.LogInfo('L''id ' || List(i) || ' est invalide, le film ne sera pas ajouté', 'Ajouter');
                		commit;
			end if;
		END LOOP;
	...
	-- Vérification de la validité du film (Si l'id en question est dans movies_ext)
	FUNCTION VerificationValidite (identifiant movie.id%type) RETURN NUMBER AS
		exist NUMBER;
	BEGIN 
